...
public class ItemServiceImpl implements  ItemService {
	...
	public boolean updateItemById(Item item) {
	    ...
	    String mergeItemDetailsQuery = 
	    	    "MERGE INTO ITEM_DETAILS d " +
	    	    "USING (SELECT ? AS ITEM_ID, ? AS DESCRIPTION, ? AS ISSUE_DATE, ? AS EXPIRE_DATE FROM DUAL) src " +
	    	    "ON (d.ITEM_ID = src.ITEM_ID) " +
	    	    "WHEN MATCHED THEN " +
	    	    "    UPDATE SET d.DESCRIPTION = src.DESCRIPTION, d.ISSUE_DATE = src.ISSUE_DATE, d.EXPIRE_DATE = src.EXPIRE_DATE " +
	    	    "WHEN NOT MATCHED THEN " +
	    	    "    INSERT (ITEM_ID, DESCRIPTION, ISSUE_DATE, EXPIRE_DATE) " +
	    	    "    VALUES (src.ITEM_ID, src.DESCRIPTION, src.ISSUE_DATE, src.EXPIRE_DATE)";
	    
	    try (Connection connection = dataSource.getConnection()) {
	        connection.setAutoCommit(false); 

	        try (PreparedStatement stmt1 = connection.prepareStatement(updateItemQuery);
	             PreparedStatement stmt2 = connection.prepareStatement(mergeItemDetailsQuery)) {
	            ...
	            if (itemDetails != null) {
	                ...
	                stmt2.setString(2, itemDetails.getDescription());
	                stmt2.setDate(3, itemDetails.getIssueDate());
	                stmt2.setDate(4, itemDetails.getExpireDate());
	                stmt2.executeUpdate();
	            }
